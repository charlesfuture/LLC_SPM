...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.') & ~strcmp(subname, '..'),
        ...
        for jj = 1:c_num,
            ...
            %%聚类 生成码本
            %%[w,h] = size(siftArr);%h = 128 w = pointNum
            %%siftFeature((iPointNum+1):(iPointNum + w),1:128) = siftArr;
            %%iPointNum = iPointNum + w;
            %%聚类 生成码本
...
%%聚类 生成码本
%[Idx,B] = kmeans(siftFeature,1024,'dist','sqEuclidean','rep',5);
%B = B';
%path = 'dictionary/Vehicle_SIFT_Kmeans_1024.mat';
%save(path, 'B');
%%聚类 生成码本
